...
func start(responseData map[string]interface{}) string {
	...
	output := util.ExecuteAndFetchWindowsCounters(strings.ReplaceAll(psScript, "\n\t", ""))
...
func parseCommandOutput(data string) map[string]string {
		...
		if match != nil {
			if key != "" && len(valueLines) > 0 {
				if systemKey, exists := mapping[key]; exists {
					result[systemKey] = strings.Join(valueLines, "\n")
				}
			}
			key = "Command-" + match[1]
			valueLines = []string{}
